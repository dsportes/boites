...
export class Compte {
  nouveau (ps, nom, cpriv) {
    ...
    const na = new NomAvatar(nom, true)
    this.mac = { }
    this.mac[na.sid] = { na: na, cpriv: cpriv }
    this.mmc = {}
    return this
  }
...
export class Avatar {
  nouveau (nom) {
    this.na = new NomAvatar(nom, true)
    this.id = this.na.id
    this.v = 0
    this.st = 0
    this.vcv = 0
    this.dds = 0
    this.cv = ['', this.na.nomc]
    this.lct = []
    return this
  }
  ...
  fromRow (row) {
    ...
    this.vcv = row.vcv
    this.dds = row.dds
    this.cv = avatarCvaType.fromBuffer(crypt.decrypter(this.nomAvatar.cle, row.cvak))
  ...
  get toRow () { // après maj éventuelle de cv et / ou lct
    ...
    const buf = rowTypes.rowSchemas.avatar.toBuffer()
    delete this.cva
    delete this.lctk
    return buf
  }
